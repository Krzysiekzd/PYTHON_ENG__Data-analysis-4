...
def main():
    ...
    ax.bar(pos,df['Percent_of_people_that_didnt_attend'],edgecolor='black',zorder=2)
    ax.set_ylim([0,100])
    ax.set_xticks(pos, labels=countries)
    ax.set_xlabel('Country',size=sizes['label'])
    ax.set_ylabel('Percent of people',size=sizes['label'])
    ax.set_title("People that didn't attend any sport event in 2011",size=sizes['title'])
